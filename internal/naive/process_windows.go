...
func attachChildLifetime(cmd *exec.Cmd) {
	...
	if err != nil {
		logger.Warningf("naive: kill-on-exit job unavailable: %v", err)
		return
	}
	h, err := windows.OpenProcess(windows.PROCESS_SET_QUOTA|windows.PROCESS_TERMINATE, false, uint32(cmd.Process.Pid))
	if err != nil {
		logger.Warningf("naive: OpenProcess for job attach failed: %v", err)
		return
	}
	defer func() { _ = windows.CloseHandle(h) }()
	if err := windows.AssignProcessToJobObject(job, h); err != nil {
		logger.Warningf("naive: AssignProcessToJobObject failed: %v", err)
	}
}
