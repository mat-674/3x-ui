...
Only enabled inbounds using **VLESS, VMess, Trojan, Shadowsocks, WireGuard, AmneziaWG, MTProto, TUIC, NaiveProxy, or Hysteria2**
appear in a subscription, ordered by their sub-sort index (TUIC, NaiveProxy and AmneziaWG are included in raw links and Clash/Mihomo profiles, but omitted from JSON endpoints; MTProto is included in raw links). Requesting `subPath`
...
routing, policy) plus a `proxy` outbound pointing at the inbound. NaiveProxy is
the exception: it emits the native `listen` + `proxy` JSON document and does
not create an Xray outbound. 3x-ui emits a **single config object for one
client and an array for several**, uses the flat `settings` form
(`address`/`port`/`id`, `level: 8`) for Xray outbounds, and strips `sockopt` from
`streamSettings`.
