...
describe('buildScriptCommand', () => {
  it('uses the master install.sh for the latest version', () => {
    expect(buildScriptCommand(base)).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/mat-674/3x-ui/master/install.sh)',
    );
  });
  ...
  it('pins a specific version by passing the tag to master install.sh', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/mat-674/3x-ui/master/install.sh) v3.4.1',
    );
  });
  ...
  it('supports the rolling dev-latest build', () => {
    ...
    expect(cmd).toBe(
      'bash <(curl -Ls https://raw.githubusercontent.com/mat-674/3x-ui/master/install.sh) dev-latest',
    );
  });
});
